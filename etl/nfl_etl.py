...
Fetches: games, players, player_game_stats, snap_counts, ftn_charting, rosters_weekly, team_game_stats
Target:  Azure SQL, nfl schema
Runs:    Nightly via GitHub Actions (.github/workflows/nightly_etl.yml)
...
Usage:
    ...
    python etl/nfl_etl.py --season 2020 --backfill  # backfill, skips ftn_charting/rosters warning
"""
...
def safe_int(series: pd.Series) -> pd.Series:
    """Convert float series (e.g. 6998.0) to nullable Int64, NaN becomes pd.NA."""
...
def bool_to_bit(series: pd.Series) -> pd.Series:
    """Map True/False strings or Python booleans to 1/0 INT. Nulls stay null."""
...
def upsert(engine, df: pd.DataFrame, table: str, schema: str, upsert_key: list[str]) -> int:
    """
    MERGE-based upsert into Azure SQL.
    Inserts new rows; updates existing rows matched on upsert_key.
    ...
    with engine.begin() as conn:
        ...
        if update_clause:
            ...
            # upsert_key covers all columns (identity-like table), just insert missing rows
            merge_sql = f"""
                MERGE [{schema}].[{table}] AS t
                USING [{temp_table}] AS s
                ON ({on_clause})
...
# ---------------------------------------------------------------------------
# 1. nfl.games  (nflreadpy.load_schedules)
# ---------------------------------------------------------------------------
def load_games(engine, season: int) -> None:
    ...
    if "ftn" in df.columns:
        df = df.rename(columns={"ftn": "ftn_id"})
...
# ---------------------------------------------------------------------------
# 2. nfl.players  (nflreadpy.load_players)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 3. nfl.player_game_stats  (nflreadpy.load_player_stats)
# ---------------------------------------------------------------------------
def load_player_game_stats(engine, season: int) -> None:
    log.info("Loading nfl.player_game_stats for season %d...", season)

    df = nflreadpy.load_player_stats(season).to_pandas()

    # Drop headshot_url (already in nfl.players)
    df = df.drop(columns=["headshot_url"], errors="ignore")

    # Rename source columns to DDL column names
    df = df.rename(columns={
        "player_id": "player_gsis_id",
    })

    # All numeric stat columns arrive as float due to nullability.
    # Cast each group according to its DDL type.
    int_cols = [
...
# ---------------------------------------------------------------------------
# 4. nfl.snap_counts  (nflreadpy.load_snap_counts)
# ---------------------------------------------------------------------------
...
def load_ftn_charting(engine, season: int) -> None:
    ...
    # Boolean columns arrive as string True/False
    bool_cols = [
        "is_no_huddle", "is_motion", "is_play_action", "is_screen_pass",
        "is_rpo", "is_trick_play", "is_qb_out_of_pocket",
...
def load_rosters_weekly(engine, season: int) -> None:
    ...
    rows = upsert(engine, df, "rosters_weekly", "nfl", ["season", "week", "team", "gsis_id"])
    log.info("  nfl.rosters_weekly: %d rows upserted", rows)


# ---------------------------------------------------------------------------
# 7. nfl.team_game_stats  (nflreadpy.load_team_stats)
# ---------------------------------------------------------------------------
def load_team_game_stats(engine, season: int) -> None:
    ...
    int_cols = [
        ...
        "pat_made", "pat_att", "pat_missed", "pat_blocked",
        "gwfg_made", "gwfg_att", "gwfg_missed", "gwfg_blocked",
        "def_interception_yards", "def_qb_hits", "def_fumbles_forced",
        "def_tackle_assists", "def_tackles_with_assist", "def_tackles_solo",
    ]
    smallint_cols = [
        "completions", "attempts", "passing_yards", "sack_yards_lost",
        "passing_air_yards", "passing_yards_after_catch",
        ...
        "penalty_yards", "punt_return_yards", "kickoff_return_yards",
        "fg_made_distance", "fg_missed_distance", "fg_blocked_distance",
        "gwfg_distance", "def_tackles_for_loss_yards",
    ]
    decimal_cols = [
        "passing_epa", "passing_cpoe", "rushing_epa", "receiving_epa",
        "def_tackles_for_loss", "def_sacks", "def_sack_yards",
...
def current_nfl_season() -> int:
    """Return the current or most recent NFL season year."""
    now = datetime.utcnow()
    # NFL seasons run Sep through Feb. If we are before June, season is prior year.
...
def main():
    ...
    parser.add_argument(
        ...
        help="Flag historical/backfill run. Suppresses large-table warnings."
    )
