"""
storage_polars.py - shared helpers for writing Parquet to Azure Blob Storage.
For use with polars DataFrames (NFL backfill scripts via nflreadpy).
...
def upload_parquet(df, blob_path: str) -> None:
    """Accepts both pandas and polars DataFrames."""
    buf = io.BytesIO()
    try:
        df.write_parquet(buf)  # polars
    except AttributeError:
        df.to_parquet(buf, index=False, engine="pyarrow")  # pandas
    buf.seek(0)
