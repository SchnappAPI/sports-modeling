...
MacBook Pro (Schnapps-MBP, pilot): live as of 2026-04-26 for limited use. Hosts a SQL Server 2022 Docker container on Colima with the full `sports-modeling` database imported from BACPAC (14.8M rows, row-count parity verified vs Azure SQL). Also hosts a second self-hosted GitHub Actions runner (`mac-runner-1`, label `mac-runner`) for the migration evaluation. Python 3.12 venv at `/Users/schnapp/venv` with the same pinned deps as the VM. ODBC Driver 18 + unixODBC installed via Microsoft's Homebrew tap. SQL credentials at `/Users/schnapp/sql-server.env` (`MSSQL_SA_PASSWORD`); only the local DB uses the SA account. Schnapp Mac MCP at `https://mac-mcp.schnapp.bet/mcp` provides shell_exec / read_file / write_file tools for remote management. No production workflow targets this host; only `mac-runner-pilot.yml` runs here. The VM remains authoritative for everything else.
