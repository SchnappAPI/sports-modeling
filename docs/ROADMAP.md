...
- **MLB Player Analysis page**. Third ADR-0003 page. Partially data-unblocked (at-bat access and career BvP access both live) but the core trend/pattern visuals need `player_trend_stats` first, so this is blocked on ETL. Could start the page shell without those visuals if the priority shifts.
...
- **MLB EV team view page**. Fourth ADR-0003 page. Data layer is ready (`mlb.player_at_bats` has all the exit-velocity fields). Could ship before Player Analysis if preferred since there's no ETL blocker.
