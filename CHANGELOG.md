...
- WARNING: this function was previously added and reverted because it was mistakenly applied in a way that removed lines incorrectly. Current implementation matches on `(player_id, stat_col, line_value)` across standard and alternate market keys. Do not remove it again without understanding why.

### UI | PlayerPageInner.tsx — column layout overhaul
- Removed `Str` column permanently. It only showed DNP; starter status moved to MIN column.
- MIN column: `*21:49` for starters (asterisk prefix), `21:49` for bench, `DNP` for did-not-play rows.
- Column order changed to: Date, Opp, MIN, PTS, FG, 3PT, REB, AST, STL, BLK, TOV, FT.
- FG column: shows `fgm/fga` per game (e.g. `7/14`). Was not previously shown. NOT a percentage.
- 3PT column: renamed from `3PM`, now shows `fg3m/fg3a` per game (e.g. `3/8`). NOT a percentage.
- `fg3a` added to `PlayerGameRow` interface and `getPlayerGames` SQL (`pbs.fg3a`).
- `buildGameSummaries` accumulates `fg3a`. `computeSplit` accumulates `fg3a`. Splits 3P% uses `fg3a` as denominator.
...
### API | queries.ts — getGrades (earlier version, same session)
- Earlier in this session, `getGrades` still used the `best_price` CTE join. This was the source of Under rows appearing in the Over tab. Fixed later in session (see 2026-04-03 entry above).

### Schema | common.daily_grades
- `ensure_tables()` in `grade_props.py` uses ADD COLUMN IF NOT EXISTS pattern to add new columns without dropping the table.
