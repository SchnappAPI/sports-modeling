...
export async function GET(req: NextRequest) {
  ...
  const lastN = context === 'all' ? 9999 : Math.max(1, parseInt(context, 10) || 20);

  // Parse period list. Empty / 'all' means no filter (sum all quarters).
  const VALID_PERIODS = ['1Q', '2Q', '3Q', '4Q', 'OT'];
  const periods = periodsRaw
    ? periodsRaw.split(',').map((p) => p.trim()).filter((p) => VALID_PERIODS.includes(p))
    : [];

  // Build IN clause for periods. Use a fixed parameterised list of up to 5.
  // If empty, no WHERE filter on period (sum all).
