...
The Mac runner exists so workflows can target the local SQL Server container instead of Azure SQL during the migration evaluation. Five workflows point at it today: `mac-runner-pilot.yml` (read-only inventory using `etl/local_db_inventory.py`), `db_inventory-mac.yml` (the production `etl/db_inventory.py` script unmodified, against the local container, for parity verification), `odds-etl-mac.yml` (a write-path mirror of `odds-etl.yml` that runs `etl/odds_etl.py` unmodified; manual dispatch only, no schedule trigger so the VM remains the sole scheduled writer against The Odds API), `compute-patterns-mac.yml` (a write-path mirror of `compute-patterns.yml` that runs `etl/compute_patterns.py` against the local container; manual dispatch only), and `nba-game-day-mac.yml` (a multi-step mirror of `nba-game-day.yml` that chains `etl/game_day_gate.py`, `etl/nba_live.py`, `etl/odds_etl.py --mode upcoming --sport nba`, `grading/grade_props.py --mode intraday`, `etl/lineup_poll.py`, and optionally `grading/grade_props.py --mode outcomes` when `skip_gate=true`; manual dispatch only, schedule trigger and the VM workflow's `gh workflow run nba-backfill.yml` step both omitted to avoid mixing DBs). Migration pattern is documented in ADR-20260426-2: production scripts read `AZURE_SQL_*` env vars; Mac workflows source `/Users/schnapp/sql-server.env` and re-export under those names so the same script runs on either runner. All scheduled and write-path workflows continue to use `runs-on: [self-hosted, schnapp-runner]`.



### Mac-hosted parallel website (`dev.schnapp.bet`)

Live as of 2026-04-26. A complete second copy of the consumer site, hosted from Schnapps-MBP, used to validate the local SQL Server container against real frontend traffic before any production cutover. Production at `schnapp.bet` is unaffected.

- `bet.schnapp.web` launchd user agent (`~/Library/LaunchAgents/bet.schnapp.web.plist`, `RunAtLoad=true`, `KeepAlive=true`) runs `next start -H 127.0.0.1 -p 3000` from `/Users/schnapp/sports-modeling/web` against the local SQL Server container at `localhost,1433`. All six web env vars are set in the plist's `EnvironmentVariables` block, including `RUNNER_URL=http://127.0.0.1:5000` and `RUNNER_API_KEY=runner-Lake4971` so the three Flask-calling routes target the local Mac Flask, not the VM. Logs at `web/web.log` and `web/web.err.log`.
- `bet.schnapp.flask` launchd user agent (`~/Library/LaunchAgents/bet.schnapp.flask.plist`, same launchd flags) runs `etl/runner.py` unmodified under `/Users/schnapp/venv/bin/python`, listening on `0.0.0.0:5000`. Auth via `RUNNER_API_KEY=runner-Lake4971` (matches the VM Flask). Mac venv added Flask 3.1.3 via `pip install flask`. Logs at `etl/flask.log` and `etl/flask.err.log`.
...
- Cloudflare ingress (`/etc/cloudflared/config.yml`): `dev.schnapp.bet -> http://127.0.0.1:3000`. The `mac-flask.schnapp.bet -> http://127.0.0.1:5000` ingress is staged at `/Users/schnapp/cloudflared-config-proposed.yml` and reload-pending; not required for normal dev operation since the dev Next.js process talks to Mac Flask via 127.0.0.1 directly.

Three Next.js routes (`api/scoreboard`, `api/live-boxscore`, `api/games`) read `RUNNER_URL` and `RUNNER_API_KEY` from `process.env` with the prior literals (`https://live.schnapp.bet`, `runner-Lake4971`) as defaults. Production stays on the VM Flask under default env; the dev plist overrides to target Mac Flask. Future cutover is a host-side env var change, not a code change. See commit `faa4d4b`.

Local DB note: the BACPAC import that populated `sports-modeling` on the Mac container carried `common.feature_flags.maintenance_mode = 1` (production was in maintenance during that snapshot per ADR-20260426-1). Flipped to 0 on the Mac container only. Azure SQL flags are independent and were already off.
